...
use App\Http\Requests\Api\Website\CheckoutRequest;
...
class CheckoutController extends Controller
{
    public function checkout(CheckoutRequest $request)
    {
        ...
        foreach ($cartItems as $cartItem) {
            ...
            $specialDiscount = $cart->specialDiscounts()->first();

            $totalAmount += $productColorSize->price_after_discount *  $cart->quantity;
            if ($specialDiscount &&  $specialDiscount->isValid()) {
                if ($specialDiscount->discount_amount) {
                    $totalAmount -= $specialDiscount->discount_amount;
                } elseif ($specialDiscount->discount_percentage) {
                    $totalAmount -= ($totalAmount * ($specialDiscount->discount_percentage / 100));
                }
            }
