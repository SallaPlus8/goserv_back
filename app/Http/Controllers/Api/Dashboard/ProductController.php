...
class ProductController extends Controller
{
    ...
    public function searchProducts(Request $request)
    {
        ...
        $filters = $request->only(['out_of_stock', 'discounted', 'nearly_out_of_stock', 'brand_id', 'category_id']);

        // Start building the query
        $query = Product::with('productColors.productColorSizes');

        // Combine the filters for "out of stock", "nearly out of stock", and "discounted"
        $query->where(function ($q) use ($filters) {
            if (isset($filters['out_of_stock'])) {
                $q->whereDoesntHave('productColors.productColorSizes', function ($subQuery) {
                    $subQuery->where('quantity', '>', 0); // Out of stock
                });
            }
            if (isset($filters['nearly_out_of_stock'])) {
                $q->whereHas('productColors.productColorSizes', function ($subQuery) {
                    $subQuery->whereBetween('quantity', [1, 5]); // Nearly out of stock
                });
            }
            if (isset($filters['discounted'])) {
                $q->whereHas('productColors.productColorSizes', function ($subQuery) {
                    $subQuery->whereHas('offers'); // Products with offers (discounted)
                });
            }
        });

        // Filter by brand (الماركة) allowing multiple brands
        if (isset($filters['brand_id'])) {
            $query->whereIn('brand_id', (array) $filters['brand_id']); // Ensure it supports an array of brand IDs
        }

        // Filter by category (القسم) allowing multiple categories
        if (isset($filters['category_id'])) {
            $query->whereIn('category_id', (array) $filters['category_id']); // Ensure it supports an array of category IDs
        }
